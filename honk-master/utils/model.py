...
def deal(number,stddev=0.01):
    if number > stddev *2:
        return torch.normal(0,std=stddev)
def truncated_normal(tensor, std_dev=0.01):
    ...
    while torch.sum(torch.abs(tensor) > 2 * std_dev) > 0:
        ...
        t.zero_()
        #print(tensor[torch.abs(tensor)>2*std_dev][0])
        #tensor.apply_(deal)
        #temp=torch.normal(t,std=std_dev)
       # print(temp[0])
        tensor=tensor.numpy()
        tensor[np.abs(tensor) > 2 * std_dev] = torch.randn(1).numpy()
...
class SpeechDataset(data.Dataset):
    ...
    def default_config():
        ...
        config["dev_pct"] = 10
        config["test_pct"] = 10
        config["wanted_words"] = ["yes"]
        config["data_folder"] = "/home/pi/data/speech_dataset"
