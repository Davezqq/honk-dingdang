...
class HonkSTT(AbstractSTTEngine):
    SLUG = "honk"
    def __init__(self,sever_endpoint):
        self._logger=logging.getLogger(__name__)
        self.endpoint= sever_endpoint
    ...
    @classmethod
    def get_config(cls):
        return config.get('honk',{})
    def transcribe_keyword(self,data):
        self.last_data=data
        self._audio_buf.append(data)
        ...
        for key in response:
           ...
           key = key.replace("_", "")
           transcribed = key
           transcribed = [transcribed.encode('utf-8')]
  	   self._logger.info('honk 识别到了：%r', transcribed) 
           self.words += [ key ]
           return transcribed
    def transcribe(self,fp):
       return 0
